            ...
            thread::spawn(move || {
                for i in 0..ITERATIONS {
                    ...
                    if i % 3 == 0 {
                        ...
                        metrics.gauge("cpu").set(thread_id as f64 + (i as f64 / 100.0));
                    } else {
...
fn show_memory_stats() {
    ...
    for i in 0..1000 {
        ...
        metrics.timer("timer_static").record(Duration::from_nanos(i as u64));
    }
    
    // Approximate memory per metric
    println!("  Counter size: {} bytes", std::mem::size_of::<metrics_lib::Counter>());
    println!("  Gauge size: {} bytes", std::mem::size_of::<metrics_lib::Gauge>());
    println!("  Timer size: {} bytes", std::mem::size_of::<metrics_lib::Timer>());
    println!("  RateMeter size: {} bytes", std::mem::size_of::<metrics_lib::RateMeter>());
